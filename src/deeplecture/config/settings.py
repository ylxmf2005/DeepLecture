...
class LLMConfig(BaseModel):
    ...
    connect_timeout: float = 30.0  # Connection timeout in seconds (default 30s for slow third-party APIs)
...
class NoteConfig(BaseModel):
    ...
    Attributes:
        default_context_mode: Default context source for note generation.
            - "auto": Automatically select based on content type and availability
            - "subtitle": Use only subtitle/transcript text
            - "slide": Use only slide/PDF text
            - "both": Use all available sources
    """

    default_context_mode: Literal["auto", "subtitle", "slide", "both"] = "auto"
