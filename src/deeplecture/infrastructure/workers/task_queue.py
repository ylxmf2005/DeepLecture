...
SQLite-backed persistence for cross-process durability.
"""
...
if TYPE_CHECKING:
    ...
    from deeplecture.use_cases.interfaces import TaskStorageProtocol
...
class TaskManager:
    """
    Task coordinator with SQLite persistence and SSE event broadcasting.

    Key design:
    - Uses SQLite storage for cross-process durability
    - In-memory cache for fast access during task execution
    - Write-through caching: persist first, then update cache
    ...
    def __init__(
        self,
        config: TaskConfig,
        task_storage: TaskStorageProtocol,
    ...
    def submit(
        ...
        *,
        ...
        # Persist to SQLite first (durability), then cache
        self._storage.save(task_entity)
        with self._lock:
            self._cache[task_id] = task_entity
    ...
    def get_task(self, task_id: str) -> Task | None:
        """Get task by ID (always from storage for cross-process consistency)."""
        self._maybe_cleanup()
        # Always read from storage to ensure cross-process consistency.
        # Multiple Flask workers share SQLite but have separate caches.
    ...
    def get_tasks_by_content(self, content_id: str) -> list[Task]:
        """Get all tasks for a content ID (always from storage)."""
        self._maybe_cleanup()
        # Always read from storage for cross-process consistency
        tasks = list(self._storage.get_by_content(content_id))
        with self._lock:
            for task in tasks:
    ...
    def update_task_progress(
        ...
        with self._lock:
            ...
            # Persist inside lock to avoid race conditions
    ...
    def recover_stale_tasks(self, stale_threshold_seconds: float = 300.0) -> int:
        """
        Mark tasks stuck in processing state as error.

        Call this on server startup to recover from unclean shutdowns.
        Only tasks that haven't been updated for longer than the threshold
        are considered stale. This prevents killing tasks that are still
        running in other processes during rolling restarts.

        Args:
            stale_threshold_seconds: Tasks with updated_at older than this
                many seconds are considered stale. Default 300s (5 minutes).

        Returns:
            Number of tasks recovered (marked as error).
        """
        from deeplecture.domain import TaskStatus
        ...
        for task in active_tasks:
            ...
            # Parse updated_at and check if task is stale
            try:
            ...
            if age_seconds < stale_threshold_seconds:
                # Task was recently updated, likely still running in another process
                logger.debug(
                    "Skipping task %s (updated %.1fs ago, threshold %.1fs)",
                    task.id,
                    age_seconds,
            ...
            logger.warning(
                "Recovering stale task %s (was %s, last updated %.1fs ago)",
                task.id,
                task.status.value,
                age_seconds,
            )
            self._fail_task_internal(
                task.id,
                f"Task interrupted by server restart (stale for {age_seconds:.0f}s)",
            )
            recovered += 1
    ...
    def _cleanup_expired_tasks(self) -> None:
        ...
        # Clean up storage
        deleted_count = self._storage.cleanup_expired(self._config.completed_task_ttl_seconds)

        # Also clean up in-memory cache of terminal tasks
        ...
        if deleted_count > 0:
            logger.info("TaskManager cleanup: removed %d expired tasks from storage", deleted_count)
...
class WorkerPool:
    ...
    def start(self) -> None:
        ...
        # Recover tasks stuck in processing state from previous run.
        # Using 60s threshold: normal tasks update progress every 1-10s,
        # so 60s without update means the task is definitely stale.
        self._manager.recover_stale_tasks(stale_threshold_seconds=60.0)
