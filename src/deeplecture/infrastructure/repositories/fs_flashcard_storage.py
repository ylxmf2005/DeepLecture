"""Filesystem implementation of FlashcardStorageProtocol.

Stores flashcard items as JSON per content/language combination at:
    content/{content_id}/flashcard/{language}.json
"""
...
class FsFlashcardStorage:
    """Filesystem-backed flashcard storage."""
    ...
    def load(self, content_id: str, language: str | None = None) -> tuple[dict[str, Any], datetime] | None:
        """Load flashcard data from filesystem.
        ...
        flashcard_dir = self._get_dir(content_id)
        if not flashcard_dir.exists():
            return None

        for file_path in flashcard_dir.glob("*.json"):
    ...
    def _load_file(self, path: Path) -> tuple[dict[str, Any], datetime] | None:
        """Load a single flashcard file."""
        try:
            content = path.read_text(encoding="utf-8")
            data = json.loads(content)
        except (OSError, json.JSONDecodeError) as exc:
            logger.warning("Failed to read flashcard %s: %s", path, exc)
            return None
    ...
    def save(self, content_id: str, language: str, data: dict[str, Any]) -> datetime:
        """Save flashcard data to filesystem.
    ...
    def exists(self, content_id: str, language: str | None = None) -> bool:
        ...
        if not flashcard_dir.exists():
            return False
        return any(flashcard_dir.glob("*.json"))
