"""Filesystem implementation of TestPaperStorageProtocol.

Stores test paper questions as JSON per content/language combination at:
    content/{content_id}/test_paper/{language}.json
"""
...
class FsTestPaperStorage:
    """Filesystem-backed test paper storage."""
    ...
    def load(self, content_id: str, language: str | None = None) -> tuple[dict[str, Any], datetime] | None:
        """Load test paper data from filesystem."""
        if language:
        ...
        test_dir = self._get_dir(content_id)
    ...
    def _load_file(self, path: Path) -> tuple[dict[str, Any], datetime] | None:
        """Load a single test paper file."""
        try:
            content = path.read_text(encoding="utf-8")
            data = json.loads(content)
        except (OSError, json.JSONDecodeError) as exc:
            logger.warning("Failed to read test paper %s: %s", path, exc)
            return None
    ...
    def save(self, content_id: str, language: str, data: dict[str, Any]) -> datetime:
        """Save test paper data to filesystem via atomic write."""
        path = self._get_path(content_id, language)
