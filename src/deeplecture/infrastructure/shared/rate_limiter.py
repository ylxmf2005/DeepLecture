...
Thread-safe rate limiting for LLM/TTS API calls.
"""
...
class RateLimiter:
    ...
    Models a bucket that refills at a fixed rate (tokens per second)
    up to a capacity. Each `acquire` call consumes tokens and blocks
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            burst_ratio: Multiplier applied to capacity for short bursts.
            now: Optional time source for testing; defaults to time.monotonic.
        """
        if max_rpm <= 0:
            max_rpm = 1
    ...
    def update_limit(self, max_rpm: int) -> None:
        ...
        Adjusts the refill rate and capacity while preserving
