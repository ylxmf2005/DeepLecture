...
class SubtitleUseCase:
    ...
    def _process_batch(
        ...
        if not subtitles:
            # Provide meaningful error message instead of "Unknown error"
            error_reason = "LLM returned empty subtitles"
            if not raw or not raw.strip():
                error_reason = "LLM returned empty response"
            elif not data:
                error_reason = "Failed to parse LLM response as JSON"
            logger.warning("No subtitles returned from LLM: %s (raw preview: %.100s)", error_reason, raw)
            return self._fallback_batch(batch, error=error_reason)
        ...
        # If all items were skipped due to invalid indices, use fallback
        if not processed and subtitles:
            error_reason = f"All {len(subtitles)} segments had invalid indices (skipped {skipped_count})"
            logger.warning("Segment mapping failed: %s", error_reason)
            return self._fallback_batch(batch, error=error_reason)
