...
class FactVerificationUseCase:
    ...
    def _build_prompt(self, subtitle_text: str, language: str) -> str:
        ...
        return f"""TASK: Fact-check video lecture content using parallel subagents. Execute immediately.

PHASE 1 - EXTRACT CLAIMS:
Analyze the subtitles and extract all verifiable factual claims. For each claim, identify:
- The exact factual statement
- The timestamp range (start, end) from the subtitle markers [n] [start-end]

PHASE 2 - VERIFY EACH CLAIM WITH SUBAGENT:
For EACH extracted claim, use the Task tool to spawn a verification subagent:
- subagent_type: "deeplecture-fact-verifier"
- prompt: Include claim_id, text, start, end times, and request verification in {language}

Launch ALL claim verifications in PARALLEL (single message with multiple Task tool calls).

PHASE 3 - SYNTHESIZE RESULTS:
After ALL subagents complete, compile their JSON results into a single report:

{{"claims": [<all subagent results>], "summary": "<overall assessment in {language}>"}}
    ...
    def _parse_result(
        ...
        for c in raw.get("claims", []):
            evidence = [
                ...
                if self._validate_url(e.get("url", ""))  # Skip invalid URLs
            ]
