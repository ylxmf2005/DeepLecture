...
export class CubismUserModel {
  ...
  public isHit(
    ...
    const viewLeft = this._modelMatrix.transformX(left);
    const viewRight = this._modelMatrix.transformX(right);
    const viewTop = this._modelMatrix.transformY(top);
    const viewBottom = this._modelMatrix.transformY(bottom);

    return viewLeft <= pointX && pointX <= viewRight && viewTop <= pointY && pointY <= viewBottom;
  }
