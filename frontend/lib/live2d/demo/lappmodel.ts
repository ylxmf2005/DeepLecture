...
export class LAppModel extends CubismUserModel {
  ...
  private setupTextures(): void {
    ...
    if (this._state == LoadStep.LoadTexture) {
      ...
      for (
        ...
        if (!textureManager) {
          console.warn('[LAppModel] TextureManager not available, skipping texture load');
          continue;
        }
  ...
  public update(): void {
    ...
    if (this._lipsync) {
      ...
      if (this._useExternalLipSync) {
        // Use external lip sync value from LipSyncManager
        value = this._externalLipSyncValue;

        // 外部リップシンク時は表情の口型を上書きする
        // External lip sync should override expression mouth values
        for (let i = 0; i < this._lipSyncIds.getSize(); ++i) {
          // Use setParameterValueById to override expression mouth values completely
          this._model.setParameterValueById(this._lipSyncIds.at(i), value);
        }
      } else {
        // Use internal WAV file handler
  ...
  public getExpressionNames(): string[] {
    const names: string[] = [];
    for (let i = 0; i < this._expressions.getSize(); i++) {
      names.push(this._expressions._keyValues[i].first);
    }
    return names;
  }
  ...
  public getMotionGroups(): { group: string; count: number }[] {
    ...
    for (let i = 0; i < this._motions.getSize(); i++) {
      const key = this._motions._keyValues[i].first;
      // key format: "group_index" (e.g., "Idle_0", "TapBody_1")
  ...
  public anyhitTest(x: number, y: number): string | null {
    if (this._opacity < 1 || !this._model || !this._modelSetting) {
      return null;
    }
  ...
  /**
   * Test if a point hits the model's rendered area (fallback when no hit areas defined)
   * @param x X coordinate to test
   * @param y Y coordinate to test
   */
  public isHitOnModel(x: number, y: number): boolean {
    ...
    for (let i = 0; i < drawableCount; i++) {
      ...
      if (transformedPoint.x >= minX && transformedPoint.x <= maxX &&
          transformedPoint.y >= minY && transformedPoint.y <= maxY) {
        return true;
      }
    }
    return false;
  ...
  /**
   * Performs a hit test with fallback
   */
  public anyHitTestWithFallback(x: number, y: number): boolean {
    return this.anyhitTest(x, y) !== null || this.isHitOnModel(x, y);
  }
  ...
  // External lip sync value (set by LipSyncManager)
  private _externalLipSyncValue: number = 0;
  private _useExternalLipSync: boolean = false;

  /**
   * Set lip sync value from external source (0-1)
  ...
  public setLipSyncValue(value: number): void {
    ...
    // Enable lip sync if not already enabled
    if (!this._lipsync) {
      this._lipsync = true;
    }
  }
  ...
  /**
   * Disable external lip sync and return to internal mode
   */
  ...
  public isExternalLipSyncEnabled(): boolean {
    return this._useExternalLipSync;
  }
  ...
  public getLipSyncValue(): number {
    return this._useExternalLipSync ? this._externalLipSyncValue : this._wavFileHandler.getRms();
  }
}
