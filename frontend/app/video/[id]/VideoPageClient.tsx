...
export default function VideoPageClient({ videoId, initialContent, initialVoiceovers }: VideoPageClientProps) {
    ...
    // Theater mode state (ephemeral UI state for layout-only wide mode)
    const [isTheaterMode, setIsTheaterMode] = useState(false);
    const toggleTheaterMode = useCallback(() => setIsTheaterMode((prev) => !prev), []);

    // Web fullscreen mode (fills browser viewport without native fullscreen API)
    const [isWebFullscreen, setIsWebFullscreen] = useState(false);
    const toggleWebFullscreen = useCallback(() => setIsWebFullscreen((prev) => !prev), []);

    // In theater mode, sidebars move below; in web fullscreen or hideSidebars, they're hidden
    const effectiveHideSidebars = hideSidebars || isWebFullscreen;
    ...
    return (
        ...
        >
            <div className={cn(
                // Theater mode: flex layout for top video + bottom panels
                isTheaterMode && "flex flex-col gap-6",
                // Normal mode: grid layout with fixed height
                !isTheaterMode && "grid gap-6 h-[130vh]",
                // Normal mode grid columns
                !isTheaterMode && (effectiveHideSidebars ? "grid-cols-1" : "grid-cols-1 md:grid-cols-3")
            )}>
                {/* Video Player & Notes Section */}
                <div className={cn(
                    "flex flex-col gap-4",
                    // Normal mode: fill grid cell height
                    !isTheaterMode && "h-full min-h-0",
                    // Normal mode with sidebar: 2/3 width
                    !isTheaterMode && !effectiveHideSidebars && "md:col-span-2",
                    // Normal mode without sidebar or theater mode: full width
                    (isTheaterMode || effectiveHideSidebars) && "col-span-1"
                )}>
                    ...
                    {/* Notes Panel - in normal mode only when not in theater mode */}
                    {!effectiveHideSidebars && !isTheaterMode && (
                        <NotesPanel
                            videoId={videoId}
                ...
                {/* Right Column: Tabs - only in normal mode (as grid column) */}
                {!effectiveHideSidebars && !isTheaterMode && (
                    <SidebarTabs
                        content={content}
                        videoId={videoId}
            ...
            {/* Bottom Section: Tabs & Notes in Theater Mode */}
            {isTheaterMode && !effectiveHideSidebars && (
