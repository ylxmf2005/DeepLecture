...
const Live2DViewer = forwardRef<Live2DViewerHandle, Live2DViewerProps>(({
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    subdelegate.onWheel(e.deltaY, localX, localY);
    setCurrentScale(subdelegate.getScale());
  }, []);

  const handlePointerDown = useCallback((e: PointerEvent) => {
    if (!subdelegateInstance || !canvasRef.current) return;
    const rect = canvasRef.current.getBoundingClientRect();
    (subdelegateInstance as { onPointBeganLocal: (x: number, y: number) => void })
      .onPointBeganLocal(e.clientX - rect.left, e.clientY - rect.top);
  }, []);

  const handlePointerMove = useCallback((e: PointerEvent) => {
    if (!subdelegateInstance || !canvasRef.current) return;
    const rect = canvasRef.current.getBoundingClientRect();
    (subdelegateInstance as { onPointMovedLocal: (x: number, y: number) => void })
      .onPointMovedLocal(e.clientX - rect.left, e.clientY - rect.top);
  }, []);

  const handlePointerUp = useCallback((e: PointerEvent) => {
    if (!subdelegateInstance || !canvasRef.current) return;
    const rect = canvasRef.current.getBoundingClientRect();
    (subdelegateInstance as { onPointEndedLocal: (x: number, y: number) => void })
      .onPointEndedLocal(e.clientX - rect.left, e.clientY - rect.top);
  }, []);
  ...
  useImperativeHandle(ref, () => ({
    setExpression: (expressionId: string) => {
      ...
      subdelegate.setExpression(expressionId);
    },
    setRandomExpression: () => {
      if (!subdelegateInstance) return;
      const subdelegate = subdelegateInstance as {
    ...
    startMotion: (group: string, index: number, priority = 2) => {
      ...
      subdelegate.startMotion(group, index, priority);
    },
    startRandomMotion: (group: string, priority = 2) => {
      ...
      subdelegate.startRandomMotion(group, priority);
    },
    getModelInfo: () => {
      if (!subdelegateInstance) return null;
      const subdelegate = subdelegateInstance as {
        getModelInfo: () => { expressions: string[]; motions: { group: string; count: number }[] };
      };
      return subdelegate.getModelInfo();
    },
    ...
    playAudioWithLipSync: async (url: string) => {
      ...
      return subdelegate.playAudioWithLipSync(url);
    },
    connectAudioForLipSync: async (audioElement: HTMLAudioElement) => {
      ...
      return subdelegate.connectAudioForLipSync(audioElement);
    },
    startMicrophoneLipSync: async () => {
      ...
      return subdelegate.startMicrophoneLipSync();
    },
    stopLipSync: async () => {
      if (!subdelegateInstance) return;
      const subdelegate = subdelegateInstance as {
      ...
      return subdelegate.stopLipSync();
    },
    pauseLipSync: () => {
      if (!subdelegateInstance) return;
      const subdelegate = subdelegateInstance as {
    ...
    isLipSyncActive: () => {
      ...
      return subdelegate.isLipSyncActive();
    },
    getLipSyncSource: (): LipSyncSource => {
      ...
      return subdelegate.getLipSyncSource();
    },
    ...
    getLipSyncValue: () => {
      ...
      return subdelegate.getLipSyncValue();
    },
  }));
  ...
  const handleMotionSyncCoreLoad = useCallback(() => {
    console.log('Live2D MotionSync Core loaded');
  ...
  const handleMotionSyncCoreError = useCallback(() => {
    console.warn('MotionSync Core not loaded - advanced lip sync unavailable');
    ...
    const initLive2D = async () => {
      try {
          ...
          if (enableMotionSync && isMotionSyncCoreLoaded && !isMotionSyncInitialized) {
            try {
              ...
              if (CubismMotionSync.startUp(motionSyncOption)) {
                ...
                console.log('MotionSync Framework initialized');
                setDebugInfo('MotionSync initialized');
              }
            } catch (e) {
              console.warn('MotionSync initialization failed:', e);
            }
          }
        }
        ...
        if (subdelegateInstance) {
          try {
            (subdelegateInstance as { release: () => void }).release();
          } catch (e) {
            console.warn('Failed to release previous subdelegate:', e);
          }
  ...
  useEffect(() => {
    ...
    const changeModel = async () => {
      try {
        ...
        const subdelegate = subdelegateInstance as {
          getLive2DManager: () => { loadModelByPath: (dir: string, file: string) => void }
        };
        subdelegate.getLive2DManager().loadModelByPath(modelDir, modelFileName);
        currentModelPathRef.current = modelPath;
