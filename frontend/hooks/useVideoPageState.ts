...
export function useVideoPageState({
    ...
    // Track the specific task ID for precise SSE matching (avoids race conditions with historical tasks)
    ...
    // Separates two concerns: (1) notifications/content refresh (run once), (2) processing reset (taskId match)
    useEffect(() => {
        let cancelled = false;
        ...
        const handleTasks = async () => {
            for (const task of Object.values(tasks)) {
                ...
                if (task.status !== "ready" && task.status !== "error") continue;

                const alreadyHandled = handledTasksRef.current.has(taskId);

                // CONCERN 1: Notifications & Content Refresh (run once per task)
                if (!alreadyHandled) {
                    // Mark as handled BEFORE async work to prevent duplicate processing on effect re-runs
                    handledTasksRef.current.add(taskId);

                    // Notify for live events, or for RECENT initial errors (within 60s)
                    // This prevents stale errors from showing toast on every page refresh
                    const isLiveEvent = task._eventType !== "initial";
                    const isRecentInitialError = (() => {
                        if (task._eventType !== "initial" || task.status !== "error") return false;
                        if (!task.updated_at) return false;
                        const updatedAt = new Date(task.updated_at).getTime();
                        const ageSeconds = (Date.now() - updatedAt) / 1000;
                        return ageSeconds < 60;
                    })();
                    if (isLiveEvent || isRecentInitialError) {
                        ...
                        if (isLiveEvent && task.status === "ready" && SUBTITLE_REFRESH_TASK_TYPES.has(task.type)) {
                            ...
                            setContent((prev) => {
                                ...
                                if (task.type === "subtitle_generation") {
                                    ...
                                    // Backend enhance_and_translate generates both enhanced and translated
                                    // subtitles simultaneously, so both statuses should be set together
                                ...
                                if (SUBTITLE_REFRESH_TASK_TYPES.has(task.type)) {
                                    setContent((prev) => {
                                        ...
                                        if (task.type === "subtitle_generation" && prev.subtitleStatus === "ready") {
                                            ...
                                            // Preserve both statuses set by enhance_and_translate
                                            if (prev.translationStatus === "ready") {
                                                preserved.translationStatus = "ready";
                                            }
                                            if (prev.enhancedStatus === "ready") {
                                                preserved.enhancedStatus = "ready";
                                            }
                                        }
                    ...
                    // Handle non-action-matching task side effects (run once)
                    if (task.type === "voiceover_generation") {
                ...
                // CONCERN 2: Processing Reset (precise taskId matching)
                // Only reset when THIS specific task completes, not any historical task of same type
                if (processingTaskId && taskId === processingTaskId) {
                    log.debug("Processing reset: task matched processingTaskId", { taskId, taskType: task.type, processingAction });
                    setProcessing(false);
                    setProcessingAction(null);
                    setProcessingTaskId(null);

                    // Task-specific side effects for processing reset
                ...
                // CONCERN 3: Fallback Processing Reset for error cases
                // When processingTaskId is null (e.g., page refresh) but processing is active,
                // reset based on task type matching to ensure UI doesn't get stuck on errors
                if (!processingTaskId && processing && processingAction && task.status === "error") {
                    const mappedAction = TASK_TO_ACTION_MAP[task.type];
                    const actionMatches = Array.isArray(mappedAction)
                        ? mappedAction.includes(processingAction)
                        : mappedAction === processingAction;
                    if (actionMatches) {
                        log.debug("Processing reset: fallback by task type match on error", {
                            taskId,
                            taskType: task.type,
                            processingAction,
                        });
    ...
    }, [tasks, videoId, processingTaskId, targetLanguage, notifyTaskComplete, voiceoverState]);
